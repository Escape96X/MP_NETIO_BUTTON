...
void saveToEEPROM(String sToSave, int startPosition) {
    char arr[sToSave.length() + 1]; // Vytvoreni pole o velikosti stringu
    strcpy(arr, sToSave.c_str()); // Prevedeni stringu do pole charakterů
    for (int i = 0; i < sToSave.length(); i++) {
        EEPROM.write(startPosition + i, arr[i]); // zapisovani do pameti po znaku
    }
    EEPROM.write(startPosition + sToSave.length(), -1); // vyprazdneni pro ukonceni retezce
    EEPROM.commit();
    delay(500);
}
...
String readEEPROM(int numberOfStart, int len) {
    String tempNetio = "";
    for (int i = 0; i < len; i++) {
        char netio = EEPROM.read(numberOfStart + i); // cteni jednotlivych znaku
        if (netio == 255) // pokud je pozice v pameti prazdna, prestane cist
            break;
        tempNetio += netio; // charekter vlozi do stringu
    }
    return tempNetio;
}
...
void connectToWiFi() {
    ...
    char *s = const_cast<char *>(ssid.c_str()); // prevod stringu na char*
...
void handleWiFiApprove() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        String badPassword = "<head><meta charset=\"UTF-8\"></head>Nesprávné heslo nebo chyba připojení <br><button onClick=\"location.href = '/wifi?ssid=";
