...
\begin{document}
	...
	\chapter{Základní informace}
		...
		\section{Platforma ESP}
			ESP jsou rodina mikročipů od společnosti \tucne{Espressif Systems} z Čínské Shangaje.
			\subsection{ESP8266}
				\subsubsection{Historie}
					ESP8266 je levný mikročip, který umí využívat WiFi. První chip, který se dostal na světlo světa byl v modulu \tucne{ESP-01}. Tento modul dokázal připojit se na WiFi síť a provádět jednoduché TCP/IP spojení. Získal si velkou oblibu u skupinek hackerů díky nízké ceně.
...
% MĚŘENÍ
	...
	\chapter{Měření spotřeby a času}
		\section{ESP8266}
		 ...
		 % Pro test byly vybrány baterie \bf{VARTA POWER AAA} o kapacitě \SI{1100}{mAh} a s napětím \SI{1.5}{V}. Jelikož známe napětí i kapacitu je možné vypočítat enegii,
		 % 	\vzorec{
			% 	E = \SI{2.2}{Ah}*\SI{3}{V}
		 % 	}
